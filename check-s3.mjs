...
async function main() {
  ...
  console.log(`Checking bucket fieldflicks-production-media for prefix ${prefix}...`);
  try {
    ...
    if (data.Contents && data.Contents.length > 0) {
      const matches = data.Contents
        .filter(item => item.Key.includes("20260812-130922"))
        .map(item => `${item.Key} (${(item.Size / 1024 / 1024).toFixed(2)} MB)`);
