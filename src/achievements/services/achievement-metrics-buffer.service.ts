...
@Injectable()
export class AchievementMetricsBufferService
  implements OnModuleInit, OnModuleDestroy
{
  ...
  private flushListeners: Array<
    (deltas: BufferedMetricDelta[]) => Promise<void>
  > = [];
    ...
    if (redisUrl || redisHost) {
      try {
        const options: RedisOptions = {
          ...
          retryStrategy: (times) =>
            times > 3 ? null : Math.min(times * 100, 1000),
        };
        ...
        this.redisClient.on('error', (err) => {
          ...
          this.logger.warn(
            `Achievement Redis buffer error (fallback active): ${err.message}`,
          );
        });

        await this.redisClient.connect().catch((err) => {
          this.logger.warn(
            `Redis connection failed (${err.message}). Using in-memory fallback buffer.`,
          );
        });
      } catch (err) {
        this.logger.warn(
          `Failed to initialize Redis metrics buffer: ${(err as Error)?.message}`,
        );
      }
    } else {
      this.logger.log(
        'No Redis host configured. Using high-performance in-memory metrics buffer.',
      );
    }
  }
  ...
  public registerFlushListener(
    listener: (deltas: BufferedMetricDelta[]) => Promise<void>,
  ): void {
  ...
  async bufferIncrement(
    ...
    if (this.isRedisConnected && this.redisClient) {
      try {
        ...
        this.logger.warn(
          `Redis bufferIncrement failed, falling back to memory: ${(err as Error)?.message}`,
        );
      }
    }
  ...
  async bufferPeak(
    ...
    if (this.isRedisConnected && this.redisClient) {
      try {
        ...
        this.logger.warn(
          `Redis bufferPeak failed, falling back to memory: ${(err as Error)?.message}`,
        );
      }
    }
  ...
  async bufferFlag(
    ...
    if (this.isRedisConnected && this.redisClient) {
      try {
        ...
        this.logger.warn(
          `Redis bufferFlag failed, falling back to memory: ${(err as Error)?.message}`,
        );
      }
    }
  ...
  async flushMetrics(): Promise<BufferedMetricDelta[]> {
    ...
    try {
      ...
      if (this.isRedisConnected && this.redisClient) {
        try {
          const pendingUsers =
            await this.redisClient.smembers('achv:pending_users');
          ...
          this.logger.error(
            `Error draining Redis metrics buffer: ${(err as Error)?.message}`,
          );
        }
      }
      ...
      if (deltas.length > 0) {
        for (const listener of this.flushListeners) {
          try {
            ...
            this.logger.error(
              `Flush listener execution failed: ${(err as Error)?.message ?? err}`,
            );
          }
        }
